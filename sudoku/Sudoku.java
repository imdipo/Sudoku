...
public class Sudoku extends JFrame {
    ...
    private void updateTimerLabel() { // timer update
        int minutes = secondsElapsed / 60;
        int seconds = secondsElapsed % 60;
        timerLabel.setText(String.format("Time: %02d:%02d", minutes, seconds));
    }

    private void pauseGame() { // buat pause
        if (!isPaused) {
            gameTimer.stop();
            isPaused = true;
    ...
    private void resumeGame() { // buat resume
        if (isPaused) {
            gameTimer.start();
            isPaused = false;
    ...
    private void applyTheme() {
        ...
        for (int row = 0; row < GRID_SIZE; row++) {
            for (int col = 0; col < GRID_SIZE; col++) {
                if (cells[row][col] instanceof Cell) {
                    ...
                    cells[row][col].setBackground(ThemeManager.getCellBackground(puzzle[row][col] == 0)); // Determine
                                                                                                          // if cell is
                                                                                                          // preset
                    cells[row][col].setForeground(ThemeManager.getCellForeground(puzzle[row][col] != 0)); // Similarly
                                                                                                          // for
                                                                                                          // foreground
                }
            }
        ...
        // update fonts
        Font themeFont = ThemeManager.getThemeFont();
        for (int row = 0; row < GRID_SIZE; row++) {
            for (int col = 0; col < GRID_SIZE; col++) {
                cells[row][col].setFont(themeFont); // update font berdasarkan tema nya
            }
        }
        ...
        for (int row = 0; row < GRID_SIZE; row++) {
            for (int col = 0; col < GRID_SIZE; col++) {
                ...
                if (puzzle[row][col] != 0) {
                    ...
                    cells[row][col].setForeground(Color.black); // buat warna font inputnya
    ...
    private void handleInput(ActionEvent e) {
        ...
        try {
            ...
            source.setText("");
            source.setBackground(Color.red); // Buat error message kalau error cell nya ganti warna
        ...
        if (Validation.isValidInput(puzzle, value, point.x, point.y)) {
            ...
            source.setText("");
            source.setBackground(Color.blue); // kalau ada yang sama kiri, kanan, atas atau bawah atau dalam 3x3 cellnya
            errorCount++;
